...
- `scopePlannerAgent.ts` calls `aiService.generateText()` instead of `this.invokeAI()` — LLM usage is NOT tracked in the `llm_usage` table for that agent. Fix to use `this.invokeAI()`.
