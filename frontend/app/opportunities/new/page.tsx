...
export default function NewOpportunityPage() {
  ...
  const [fileRfpTitle, setFileRfpTitle] = useState<string>('');
  const [fileClientName, setFileClientName] = useState<string>('');
  const [fileTherapeuticArea, setFileTherapeuticArea] = useState<string>('');
  const [fileDeadline, setFileDeadline] = useState<string>('');
      ...
      <div className="mx-auto px-6 py-6">
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="relative pl-16 mb-8">
            {/* Connecting Line */}
            <div className="absolute left-6 top-12 bottom-0 w-0.5 -mb-8 bg-slate-200" />

            {/* Step Icon Circle */}
            <div className="absolute left-0 top-0 w-12 h-12 rounded-full flex items-center justify-center z-10 shadow-lg bg-primary text-white ring-4 ring-primary/20">
              <FileText className="w-6 h-6" />
            </div>

            {/* Step Card - Upload Form */}
            <div className="rounded-xl p-6 shadow-sm bg-white border-2 border-primary">
              <div className="flex justify-between items-start mb-4">
                <div className="flex-1">
                  ...
                  <p className="text-xs text-slate-500">Upload RFP document or paste text to begin processing</p>
                </div>
              </div>
              ...
              {mode === 'text' ? (
                ...
                <form onSubmit={handleFileSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <label
                      htmlFor="file-upload"
                      className="block border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-lg p-8 text-center cursor-pointer hover:border-ps-primary-400 transition-colors"
                    >
                      {file ? (
          ...
          {workflowSteps.slice(1).map((step, index) => {
            ...
            return (
              <div key={step.id} className="relative pl-16 mb-8">
                ...
                {/* Step Icon Circle */}
                <div className="absolute left-0 top-0 w-12 h-12 rounded-full flex items-center justify-center z-10 shadow-sm bg-slate-100 text-slate-400 border-2 border-white">
                  <Icon className="w-6 h-6" />
                </div>

                {/* Step Card */}
                <div className="rounded-xl p-6 shadow-sm bg-white border border-slate-200 opacity-70">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="px-2 py-0.5 text-[10px] font-black rounded-lg uppercase tracking-wider bg-slate-50 text-slate-400">
                          Locked
                        </span>
                      </div>
